...
function AuthSocialButtons({ isLoading, callbackUrl }: AuthSocialButtonsProps) {
  return (
...
function AuthSignIn({ callbackUrl, onForgotPassword, onSignUp }: AuthSignInProps) {
  ...
  return (
    ...
    >
      ...
      <AuthSeparator />
      <AuthSocialButtons isLoading={formState.isLoading} callbackUrl={callbackUrl} />
...
function AuthSignUp({ callbackUrl, onSignIn }: AuthSignUpProps) {
  const router = useRouter();
  const [formState, setFormState] = React.useState<FormState>({
    isLoading: false,
    error: null,
...
function AuthForgotPassword({ onSignIn, onSuccess }: AuthForgotPasswordProps) {
  ...
  const onSubmit = async (_data: ForgotPasswordFormValues) => {
    setFormState((prev) => ({ ...prev, isLoading: true, error: null }));
    try {
      // Password reset API not yet implemented; show success UI for now.
